...
到此，算是把官方文档中 🐶**个人觉得比较重要** 的内容过了一遍，相信读者们对Flutter的认知又更深了。剩余部分内容大部分是跟 **Flutter插件(库)** 有关的没，如：如何使用GPS传感器 → 使用geolocator插件；如何使用相机 → 使用image\_picker插件；如何使用 Shared Preferences → Shared\_Preferences 插件 等等...；还有一部分和 **工程结构**、**资源文件** 相关的。**抛砖引玉，点到为止**，后续会进行相应的专题学习，敬请期待😁~
