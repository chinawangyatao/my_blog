...
* **dispose**\* ：路由将被立即销毁。
* **disposing**：\_RouteEntry实例正在等待其小部件子树先销毁。在等待销毁期间，它被存储在\_entryWaitingForSubTreeDisposal中。
* **disposed**：路由已被销毁。
...
🤣 见名知意，就是 **刷新\_history列表**，点开方法打下断点，上面创建初始路由时设置了 **initialState: \_RouteLifecycle.add**，这里自然是走的 add 逻辑：
